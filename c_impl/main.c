  ...
  while(true) {
    ...
    switch (c) {
      ...
      case 'a':
        ...
        printf("Entry name: "); fgets(name, 128, stdin);
        printf("User name: "); fgets(user, 128, stdin);
        printf("Password: "); fgets(pass, 128, stdin);
        printf("Description (public): "); fgets(desc, 256, stdin);
        entry->name = name;
        entry->user = user;
        entry->pass = pass;
        entry->desc = desc;
